...
<xsl:template match="artwork[@src and starts-with(@type,'image/')]">
  <img xmlns="http://www.w3.org/1999/xhtml" src="{@src}" alt="{.}">
  ...
  <xsl:if test="not($xml2rfc-private)">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <p xmlns="http://www.w3.org/1999/xhtml" class="error"><xsl:value-of select="$node-set-warning"/></p>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:if>
...
<xsl:template match="xref[node()]">
  ...
  <xsl:choose>
    ...
    <xsl:when test="@x:fmt and @x:fmt!='none'">
      ...
      <span xmlns="http://www.w3.org/1999/xhtml" class="error">unknown xref/@x:fmt extension: <xsl:value-of select="@x:fmt"/></span>
    </xsl:when>
...
<xsl:template match="xref[not(node())]">
  ...
  <xsl:if test="count($node)=0 and not(ancestor::ed:del)">
    ...
    <span xmlns="http://www.w3.org/1999/xhtml" class="error">Undefined target: <xsl:value-of select="@target"/></span>
  </xsl:if>
  ...
  <xsl:choose>
      ...
      <xsl:if test="@x:sec">
        <xsl:choose>
          <xsl:when test="@x:fmt='()'">
            ...
            <xsl:choose>
              <xsl:when test="$href!=''">
                <a xmlns="http://www.w3.org/1999/xhtml" href="{$href}">Section <xsl:value-of select="@x:sec"/></a>
              </xsl:when>
              <xsl:otherwise>Section <xsl:value-of select="@x:sec"/></xsl:otherwise>
            </xsl:choose>
            <xsl:text>)</xsl:text>
    ...
    <xsl:otherwise>
      ...
      <span xmlns="http://www.w3.org/1999/xhtml" class="error">xref to unknown element: <xsl:value-of select="name($node)"/></span>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
    ...
    <xsl:otherwise>
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <a xmlns="http://www.w3.org/1999/xhtml" href="#{$target}"><xsl:value-of select="$title"/></a>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:otherwise>
  </xsl:choose>
...
<xsl:template match="x:ref">
  ...
  <xsl:choose>
    <xsl:when test="$target">
      <a xmlns="http://www.w3.org/1999/xhtml" href="#{$target/@anchor}"><xsl:value-of select="."/></a>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="insert-diagnostics">
  ...
  <xsl:if test="$badAnchors">
    <p xmlns="http://www.w3.org/1999/xhtml" class="warning">
  ...
  <xsl:if test="$badTargets">
    <p xmlns="http://www.w3.org/1999/xhtml" class="error">
...
<xsl:template name="warning">
  <xsl:param name="msg"/>
  <xsl:param name="msg2"/>
  <div xmlns="http://www.w3.org/1999/xhtml" class="error">WARNING: <xsl:value-of select="$msg"/></div>
