...
HTML = {
    _ROOT: ('<html><body><pre>', '</pre></body></html>'),
    _STRING: ('<span style="color:green;font-weight:bold" >', '</span>'),
    _COMMENT:('<font color="#008000">', '</font>'),
    _NAME:  ('<span style="color:blue;font-weight:bold">', '</span>'),
    _KEYWORD:('<span style="color:red;font-weight:bold">', '</span>'),
    _TEXT:  ('', '')
    } 
...
class XmlFormatSaxHandler(ContentHandler):
    ...
    def characters(self, ch):
        if self._in_entity: 
            return
        elif not self._in_cdata: 
            ch = escape(ch)
            self._out.write('%s%s%s' % (self._o_d[_TEXT][0], ch.encode(self._cod), self._o_d[_TEXT][1]))
...
def run(args):
    ...
    for file in args:
        ...
        h = XmlFormatSaxHandler(1, dest, encod)
        h.set_format(format)
